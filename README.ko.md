...
- **스마트 플래닝** — `/plans`로 프로젝트를 분석하고, 필요한 도구를 설치한 뒤 바로 플래닝 모드로 진입합니다.
...
skillless의 핵심 기능입니다. 프로젝트를 분석하고, 도구를 추천하고, 플래닝 모드까지 한번에:

```
/plans React + Supabase로 대시보드 만들어줘
```

실행되는 과정:
1. `package.json`, `tsconfig.json` 등 프로젝트 파일 스캔
2. 기술 스택 감지 (React, Supabase, TypeScript 등)
3. 매칭되는 도구를 모든 소스에서 검색
4. 추천 목록을 보여주고 선택하면 설치
5. 설치된 도구를 활용해 플래닝 모드 진입

```
감지된 기술: React, TypeScript, Supabase
이미 설치됨: typescript-expert

추천 도구:
| # | 이름                  | 소스       | 설명                          |
|---|----------------------|----------|-------------------------------|
| 1 | react-best-practices | skills.sh | React 패턴과 hooks             |
| 2 | supabase-automation  | skills.sh | Supabase 인증 및 DB 패턴       |

설치할 번호를 선택하세요 (1,2 / skip):
```

#### 그냥 평소처럼 작업하세요

아무것도 안 해도 됩니다. 특정 프레임워크나 도구로 작업할 때 관련 능력이 있으면 자연스럽게 추천됩니다:
